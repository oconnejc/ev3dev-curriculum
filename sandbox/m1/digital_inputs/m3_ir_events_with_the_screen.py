...
def main():
    ...
    # : 3. Create a remote control object for channel 1. Add lambda callbacks for:
    ...
    rc1.on_red_up = lambda state: handle_red_up_1(button_state=state,dc=dc)
    rc1.on_red_down = lambda state: handle_red_down_1(button_state=state, dc=dc)
    rc1.on_blue_up = lambda state: handle_blue_up_1(button_state=state, dc=dc)
    rc1.on_blue_down = lambda state: handle_blue_down_1(button_state=state, dc=dc)



    # : 5. Create remote control objects for channels 2, 3, and 4. Add lambda callbacks for on_red_up to each one:
    ...
    rc2.on_red_up = lambda state: handle_red_up_2(button_state=state, dc=dc)

    rc3 = ev3.RemoteControl(channel=3)
    rc3.on_red_up = lambda state: handle_red_up_3(button_state=state, dc=dc)

    rc4 = ev3.RemoteControl(channel=4)
    rc4.on_red_up = lambda state: handle_red_up_4(button_state=state, dc=dc)
    ...
    while dc.running:
        # : 4. Call the .process() method on your channel 1 RemoteControl object, then review and run your code.
        #   Review the handle functions below to see how they draw to the screen.  They are already finished.
        rc1.process()
        # : 6. Call the .process() method on your channel 2 - 4 RemoteControl objects and demo your code.
        #   Review the handle functions below to see how they draw to the screen.  They are already finished.
